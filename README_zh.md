...
在独立的 git worktree 中启动编码 Agent，每个 Agent 运行在各自的 tmux 会话里。Agent 独立完成编码、提交、推送，后台还有个贴心的小监控，检测完成状态后还会通过企业微信给你发消息通知～ 🎉

支持 Claude Code、OpenClaw、Codex 等 AI 编码助手。
...
- `tmux` — 用于会话管理
- `jq` — 用于 JSON 任务追踪
- `claude`（Claude Code CLI）或任意兼容的 AI 编码助手
